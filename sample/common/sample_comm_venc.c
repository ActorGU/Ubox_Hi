...
#elif RTSP_FIFO
    const char *fifo_name = "/mnt/live555/stream_chn0.265";
    int pipe_fd = -1;
    int data_fd = -1;
    int ret = 0;
            ...
            for (i = 0; i < s32ChnTotal; i++)
            {
                if (FD_ISSET(VencFd[i], &read_fds))
                {
                    ...
                    *******************************************************/
                    ...
                    {
                        // printf("num %d stream len=%d\t,offset=%d\n",x,stStream.pstPack[x].u32Len,stStream.pstPack[x].u32Offset);
                        numbytes = stStream.pstPack[x].u32Len - stStream.pstPack[x].u32Offset;
                        numkbytes = numbytes/1024;
                        for (int y = 0; y < numkbytes; y++)
                        {
                            memcpy(buf,stStream.pstPack[x].pu8Addr + stStream.pstPack[x].u32Offset + 1024*y,1024);
                            sendto(server_fd, buf, 1024, 0, (struct sockaddr*)&ser_addr, sizeof(ser_addr));
                            // printf("send 1k bytes\n");
                        }
                        if(numbytes-1024*numkbytes>0)
                        {
                            memcpy(buf,stStream.pstPack[x].pu8Addr + stStream.pstPack[x].u32Offset + 1024*numkbytes,numbytes-1024*numkbytes);
                            sendto(server_fd, buf, numbytes-1024*numkbytes, 0, (struct sockaddr*)&ser_addr, sizeof(ser_addr)); 
                            // printf("send %d bytes\n",numbytes-1024*numkbytes);                           
                        }
                    }
                    
                    strcpy(buf, "Hello Server\n");
                    sendto(server_fd, buf, BUFF_LEN, 0, (struct sockaddr*)&ser_addr, sizeof(ser_addr));  //发送信息给client，注意使用了clent_addr结构体指针
    #elif RTSP_FIFO
                    int per = 4096;
                    // 向FIFO文件写数据                  
                    for (int x = 0; x < stStream.u32PackCount; x++)
                    ...
                    #if 1
                        numbytes = stStream.pstPack[x].u32Len - stStream.pstPack[x].u32Offset;
                        ret = write(pipe_fd, stStream.pstPack[x].pu8Addr + stStream.pstPack[x].u32Offset, numbytes);
                        if (ret == -1)
                        {
                    ...
                    #else                    
                        numbytes = stStream.pstPack[x].u32Len - stStream.pstPack[x].u32Offset;
                        numkbytes = numbytes/per;
                        for (int y = 0; y < numkbytes; y++)
                        {
                            ret = write(pipe_fd, stStream.pstPack[x].pu8Addr + stStream.pstPack[x].u32Offset + per*y, per);
                            if (ret == -1)
                            {
                                fprintf(stderr, "Write error on pipe\n");
                            }
                            // printf("send per bytes\n");
                        }
                        if(numbytes-per*numkbytes>0)
                        {
                            ret = write(pipe_fd, stStream.pstPack[x].pu8Addr + stStream.pstPack[x].u32Offset +  per*numkbytes, numbytes-per*numkbytes);
                            if (ret == -1)
                            {
                                fprintf(stderr, "Write error on pipe\n");
                            }
                            // printf("send %d bytes\n",numbytes-per*numkbytes);                           
                        }  
                    #endif                      
                    }
